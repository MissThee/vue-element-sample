  ...
  <div class="animate" style="overflow: hidden;height: 100%;">
     ...
     <el-dialog :title="dialogTitle[formDialogStatus]" :visible.sync="formDialogVisible" :close-on-click-modal="false" append-to-body>
      <el-form :rules="rules" ref="formData" size="mini" :model="form" label-width="80px">
        <el-form-item label="上级分组" prop="loginId">
           <el-cascader
             :props="cascaderProps"
             placeholder="上级分组"
        ...
        <el-form-item label="分组名称" prop="loginId">
...
<script>
  import {createPost, deletePost, fetchPostList, updatePost} from 'src/api/post'
            ...
            createElement('el-button', {
              attrs: {
                ...
                click: function () {
                  ...
                  let path = self.getNodePath(self.form.parentId, temp[0]);
                  self.postList = self.setDisabledNode(temp, null);
              ...
              on: {
                click: function () {
                  if (data.id === 0) {
                    ...
                    let path = self.getNodePath(self.form.id, temp[0]);
                    let id = path.pop();
                    self.postList = self.setDisabledNode(temp, id);
                    self.currentNodePath = path;
                  }
                }
              }
