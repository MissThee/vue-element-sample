    ...
    <el-table ref="table1" :v-loading="listLoading" :data="userTableData" :height="clientHeight-32" stripe :default-sort="{prop: 'name', order: 'ascending'}" border>
      ...
      <el-table-column fixed="right" label="操作" width="106">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" style="color:red;" @click="handelDelete(scope.row.id,scope.row.loginId,scope.row.isAdmin)" plain><i class="el-icon-circle-close-outline "></i>删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
