<template>
  <div class="animate" style="overflow: hidden;height: 100%;">
    <el-row class="layout-fit">
      <el-col :span="leftSpan" class="layout-fit">
        <el-row>
          <div :gutter="20" class="grid-tool-bar">
            ...
            <el-button :icon="showMap?'el-icon-arrow-right':'el-icon-arrow-left'" :style="{float:'right',marginRight:'-5px',fontWeight: 'bold',width:showMap?'15px':'',paddingLeft:showMap?'5px':''}" size="mini" type="primary" @click="handleSwitchMap()" plain>{{showMap?'':'地图'}}</el-button>
          </div>
        </el-row>
        <el-table :key="Math.random()" :data="importantCompanyTableData" @row-click="handleRowClick" :height="clientHeight-32" stripe :default-sort="{prop: 'name', order: 'ascending'}" border style="width: 100%;height:100%">
          <el-table-column :width="50" key="index" type="index" label="序号"></el-table-column>
          <el-table-column :width="140" key="area" v-if="!showMap" prop="area" label="区域" sortable></el-table-column>
          <el-table-column :width="140" key="protectedRange" v-if="!showMap" prop="protectedRange" label="消防保护范围" sortable></el-table-column>
          <el-table-column :width="140" key="type" v-if="!showMap" prop="type" label="类别" sortable></el-table-column>
          <el-table-column :min-width="140" key="importantCompany" prop="importantCompany" label="重点单位" sortable></el-table-column>
          <el-table-column :width="140" key="address" v-if="!showMap" prop="address" label="具体位置" sortable></el-table-column>
          <el-table-column :width="140" key="level" v-if="!showMap" prop="level" label="级别" sortable></el-table-column>
          <el-table-column :width="140" key="responsePerson" v-if="!showMap" prop="responsePerson" label="消防责任人" sortable></el-table-column>
          <el-table-column :width="140" key="contactInformation" v-if="!showMap" prop="contactInformation" label="联系方式" sortable></el-table-column>
          <el-table-column key="operation" fixed="right" label="操作" width="106">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" style="color:red;" @click="handelDelete(scope.row.id,scope.row.importantCompany)" plain><i class="el-icon-circle-close-outline "></i>删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <el-col v-if="ifMap" v-show="showMap" :span="rightSpan" class="layout-fit">
        <div id="container" class="layout-fit"></div>
      </el-col>
    </el-row>
